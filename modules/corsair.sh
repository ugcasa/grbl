...
corsair_config="$GURU_CFG/$GURU_USER/corsair.cfg"
...
# Check keyboard driver is available, app and pipes are started and launch those if needed
    ...
    if [[ ${GURU_CORSAIR_KEYBOARD_ID} ]]; then
            gr.msg -v2 -n -c green "configured, "
        if lsusb | grep -q ${GURU_CORSAIR_KEYBOARD_ID}; then
    ...
    if [[ ${GURU_CORSAIR_KEYBOARD_ID} ]]; then
        gr.msg -v2 -n -c green "configured, "
        if lsusb | grep -q ${GURU_CORSAIR_MOUSE_ID}; then
            gr.msg -v2 -c green "connected"
        else
            gr.msg -c dark_grey "disconnected"
        fi
    ...
    gr.msg -v1 -c white "If not responsive try to restart daemon by '$GURU_CALL corsair restart' "
...
# construct corsair configuration rc
    ...
    if ! [[ -f $corsair_config ]] ; then
        ...
        corsair_config="$GURU_CFG/corsair.cfg"
