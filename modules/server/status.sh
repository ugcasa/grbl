...
# source $GURU_BIN/telegram.sh telegram.msg
telegram.msg () {
...
while true ; do
    server.status zeukkari.dev ping
    server.status zeukkari.dev https wekan wiki/wiki gitea/explore/repos
    server.status zeukkari.dev ssh 22 casa
    server.status 192.168.10.106 mqtt 1883
    # server.status 192.168.1.10 ping
    # server.status localhost:8282 http wekan
    # server.status 192.168.1.10 ssh 22 casa
    sleep 300
done
