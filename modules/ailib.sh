#!/bin/bash
## based on chatGPT-shell-cli https://github.com/0xacx/chatGPT-shell-cli
## thanks 0xacx https://github.com/0xacx
## modified to be able use it as an library by sourcing it
...
ailib.list_models() {
	...
	ailib.handle_error "$models_response"
...
ailib.add_assistant_response_to_chat_message() {
	local escaped_response_data="$1"
	# add response to chat context as answer
	...
	while $running; do
		...
		if [ -z "$pipe_mode_prompt" ]; then
			if [ $MULTI_LINE_PROMPT = true ]; then
				...
				prompt=$(ailib.escape "$input_from_temp_file")
			else
				echo -e "\nEnter a prompt:"
				read -e prompt
			fi
		...
		if [[ $prompt =~ ^(exit|q)$ ]]; then
			...
			escaped_prompt=$(ailib.escape "$prompt")
			escaped_prompt=${escaped_prompt#command:}
			request_prompt=$COMMAND_GENERATION_PROMPT$escaped_prompt
			ailib.build_user_chat_message "$request_prompt"
			response=$(ailib.request_to_chat "$chat_message")
			ailib.handle_error "$response"
			...
			ailib.add_assistant_response_to_chat_message "$(ailib.escape "$response_data")"
			...
			# escape quotation marks, new lines, backslashes...
			request_prompt=$(ailib.escape "$prompt")
			...
			if [ "$CONTEXT" = true ]; then
				ailib.build_chat_context "$request_prompt"
			fi

			response=$(ailib.request_to_completions "$request_prompt")
			ailib.handle_error "$response"
			...
			if [ "$CONTEXT" = true ]; then
				ailib.maintain_chat_context "$(ailib.escape "$response_data")"
			fi
